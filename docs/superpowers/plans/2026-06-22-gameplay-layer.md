...
```
Notes (full detail in spec §5.1/§5.4): `enemies_spawn(es, wave)` indexes `wave_table[(wave>=1?wave-1:0)]`, looping at index 15 for wave>16. **Pattern source:** waves 1–16 use the wave row's `pattern` field (the approved per-wave map — NOT rng-picked); only wave>16 rng-picks (reroll while `== last_pattern`). `last_pattern` (a `static u8`) is set to whichever pattern was used. RNG order: (1) wave>16 only — draw(s) to pick pattern (reroll while `==last_pattern`); waves 1–16 draw nothing here. (2) place `count` positions from the chosen pattern's `static const` table, levels first-bounce-then-chase-then-hunter from the wave row (no rng). (3) **two `rng_byte()` per BOUNCER** (not per slot — chasers/hunters ignore dx/dy) for its `dx/dy`. Counts are clamped to `MAX_ENEMIES` (stays 6 this task; raised only in Task 9). Delete `SX[]/SY[]`, `pick_level`, the kill thresholds. Pattern tables hold up to 8 positions; only `count` are used.
