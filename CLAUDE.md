...
- **Timex build:** `make timex` → `build/aw-<version>-timex.tap`. Requires z88dk at `~/Programowanie/z88dk` or on `PATH`. Compiles the C sources + hand asm with `zcc +zx -SO3 -clib=sdcc_iy`. ORG defaults to `0x8000` (no `-zorg`).
- **ZX Spectrum 128K build:** `make zx128` → `build/aw-<version>-zx128k.tap`. It defines `ZX128_PAGE_FLIP` and `ZX_SINCLAIR_DUAL_STICK`: RAM page 7 is kept banked into `$C000` (shadow screen), bit 3 of port `$7FFD` flips between page 5/page 7, and "two joysticks" means Sinclair 1/2. Resident code ORGs at `$6000` (`-pragma-define:CRT_ORG_CODE=24576`) — the `$6000–$7FFF` hole is free on 128K because screen B lives in page 7, not at `$6000` as on Timex — keeping code/data/BSS below `$C000` (gated by `tools/check_zx128_layout.py`, `SP=$C000`). **Full Timex parity:** the ~10 KB PT3 tune ships as a trailing headerless tape block (appended by `tools/make_tape_block.py`) that the running program loads into RAM **bank 4** at `$C000` (`zx128_load_tune`, via ROM `LD-BYTES` at `$0556`); the player is resident and the 50 Hz IM2 ISR pages bank 4 into `$C000` for each tick using a `$7FFD` software shadow in `zx128_page.asm`. The IM2 vector table lives in page-7 free RAM at `$F000`. Gameplay and enemy sprites also match Timex now (the old `ZX128_NO_MUSIC` memory-reduced variants are gone).
- **ZX Spectrum 48K build:** `make zx48` → `build/aw-<version>-zx48k.tap`. It defines `ZX48_SINGLE_BUFFER` and `ZX_SINCLAIR_DUAL_STICK`: no SCLD writes, screen B aliases screen A, `scld_present()` is just `HALT`, and "two joysticks" means Sinclair 1/2 rather than TS2068 AY joystick ports.
- **Run / verify:** `make run-tc2048` launches `build/aw-<version>-timex.tap` in **ZEsarUX** as a `TC2048` (`--enabletimexvideo`, plus `--configfile tools/zesarux.rc` which carries `--joystickemulated Kempston` and the host pad mapping; refresh it from `~/.zesaruxrc` with `make zesarux-config`). `make run-tc2068` launches the Timex build as `TC2068`. `make run-zx128` launches `build/aw-<version>-zx128k.tap` as `128k`. `make run-zx48` launches `build/aw-<version>-zx48k.tap` as a plain `48k`. ZEsarUX is the preferred Timex emulator here — not Fuse (the macOS Fuse build has flaky HID joystick support and no headless screenshot). Controls: move with `5/6/7/8` (cursor) or a Kempston pad; fire with `0` or `Q W E / A D / Z X C`.
...
- **Target-only hardware:** `scld.c` (SCLD double-buffer kernel), `sprite.c` (8×8 pre-shifted OR-blitter + cheap bullet dots), `blit.asm` (hand-written hot drawing inner loop), `sprites.c` (8×8 pixel-art data), `sprite_art.c` (per-target placement of the pre-shift tables). Verified visually in the emulator + by cycle-counting with `z88dk-ticks`.
- **Target-only presentation:** `background.c` (generated floor + frame + ULA border; `bg_attr()` is the restore oracle every effect asks before repainting a cell), `fx.c` (hit pops, death fireball, spawn telegraph — attribute-only), `text.c` (ROM-font blitting), `hud.c` (cached HUD widgets incl. the score), `title.c`, `gameover.c`.
...
- **`main.c`** is the glue: the 50 Hz game loop, title→playing state, input → logic → incremental render → present. It is deliberately the only file that touches `prev[2][MAX_DRAW]` and the erase/redraw hot path.
- **AY sound asm:** `ay_ports.asm` (machine detection + the latched port pair — 128K `0xFFFD/0xBFFD` vs TS2068 `0xF5/0xF6`), `ay_sfx.asm` (FX-only channel C), `pt3_glue.asm` (the `asm_vt_hardware_out` override, `sfx_merge`, IY-safe player wrappers), `music_im2.asm` (IM2 table + the 50 Hz ISR), `pt3prom.asm` (**vendored verbatim** from z88dk — do not reformat or split).
...
- **ZX Spectrum 128K page flipping lives in its separate build.** `make zx128` ORGs code at `$6000`, keeps resident code/data/BSS below `$C000`, sets `SP=$C000`, validates with `tools/check_zx128_layout.py`, maps RAM page 7 into `$C000`, and flips display bit 3 on port `$7FFD`. The page values must preserve ROM1 (`$17` for screen 5, `$1F` for screen 7); using `$07/$0F` switches to ROM0 and can reboot under IM1. The PT3 tune is banked into RAM **bank 4** and paged into `$C000` only around each player tick (`zx128_tune_in`/`zx128_tune_out`); those writes also keep ROM1 set (`$14` = ROM1+bank4). All `$7FFD` writes go through `zx128_page.asm`, which holds the software shadow the ISR reads (the port is write-only).
...
- **`scld.h` vs `scld_geom.h`.** The two pure scanline-geometry helpers (`scld_scanline`, `scld_next_scanline`) live in `scld_geom.h`, not `scld.h`. They are `static inline`, and sdcc emits a dead out-of-line copy of each (~95 B) into *every* translation unit that merely sees them — with a dozen modules including `scld.h` that was ~460 B of dead weight and broke the ZX48 stack-gap check. Include `scld_geom.h` only where scanline addresses are actually computed in C.
