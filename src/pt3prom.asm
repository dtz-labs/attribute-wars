; pt3prom.asm -- VENDORED ProTracker-3 player (Vortex Tracker II r.7 ROM build).
;
; Copied verbatim from z88dk: libsrc/psg/ay/vt2/PT3PROM.asm. Vendored here because
; the z88dk psg/vt2 player only ships for the classic clib, and this project
; builds with -clib=sdcc_iy where neither <psg/vt2.h> nor the library link. The
; file is pure z80asm and self-contained: its ONLY externs are asm_vt_hardware_out
; / _A0, which src/music_ay.asm provides (routing register writes through the
; runtime-detected AY ports, so both 128K and TS2068 play). We call its
; asm_VT_INIT (HL=module addr) / asm_VT_PLAY / asm_VT_MUTE entry points via the
; thin IY-preserving wrappers in music_ay.asm. Original author credit follows.
; ---------------------------------------------------------------------------
